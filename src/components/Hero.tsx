...
export const Hero = () => {
  ...
  return <section className="relative min-h-[600px] bg-gradient-to-b from-accent/5 to-white/80">
        ...
        <div className="grid md:grid-cols-12 gap-6 items-center min-h-[600px]">
          ...
          duration: 0.6
            ...
            duration: 0.3
              ...
              <img src="https://images.unsplash.com/photo-1445205170230-053b83016050" alt="Trendy Stores" className="w-full h-40 object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent p-4 flex flex-col justify-end">
                ...
                <Link to="/products" className="text-white hover:text-accent-new transition-colors text-sm font-medium flex items-center gap-1">
                  Shop Now
            ...
            duration: 0.3
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="font-bold text-lg text-orange-700">NOVA CLUB</h3>
              </div>
          ...
          duration: 0.6
            ...
            <div className="relative w-full h-full rounded-2xl overflow-hidden">
              <img src={heroContent[currentImageIndex].image} alt="Fashion" className="w-full h-full object-cover" />
              ...
              <div className="absolute bottom-8 left-8 right-8 text-white">
                ...
                duration: 0.4
                  ...
                  <Link to="/products" className="inline-flex items-center gap-2 px-6 py-2 bg-white text-black rounded-full mt-4 hover:bg-accent-new hover:text-white transition-colors">
                    Shop Now
                    <ArrowRight className="w-4 h-4" />
                  </Link>
                </motion.div>
              ...
              {/* Navigation Buttons */}
              <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm hover:bg-white/30 transition-colors flex items-center justify-center text-white">
                <ChevronLeft className="w-6 h-6" />
              </button>
              <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm hover:bg-white/30 transition-colors flex items-center justify-center text-white">
                <ChevronRight className="w-6 h-6" />
              </button>

              {/* Dots */}
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                {heroContent.map((_, index) => <button key={index} onClick={() => setCurrentImageIndex(index)} className={`w-2 h-2 rounded-full transition-all ${index === currentImageIndex ? 'bg-white w-4' : 'bg-white/50'}`} />)}
              </div>
            </div>
          </motion.div>
          ...
          duration: 0.6
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-xl p-4 shadow-lg">
              ...
              <div className="grid grid-cols-2 gap-3">
                {trendingProducts.map((product, index) => <div key={index} className="group">
                    <div className="relative overflow-hidden rounded-lg">
                      <img src={product.image} alt={product.tag} className="w-full aspect-square object-cover group-hover:scale-110 transition-transform duration-300" />
                      <div className="absolute top-2 right-2">
                        <span className={`text-xs px-2 py-1 rounded-full ${product.status === "Rising 66%" ? "bg-gradient-to-r from-blue-500 to-blue-600 text-white" : "bg-gradient-to-r from-purple-500 to-pink-500 text-white"}`}>
                          {product.status}
                        </span>
                      </div>
                    </div>
